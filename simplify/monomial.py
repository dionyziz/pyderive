...
def unifyMonomialFactors(a, b):
    ...
    * A list of astNodes if the unification yielded multiple terms (e.g. by expansion)
    ...
    if (
        isinstance(a, astPlus)
        or isinstance(b, astPlus)
        or isinstance(a, astMinus)
        or isinstance(b, astMinus)
    ):
    ...
    # x^n * x^m = x^(n + m)
    if (
        isinstance(a, astPower)
        and isinstance(b, astPower)
        and isinstance(a.left, astVar)
        and isinstance(b.left, astVar)
        and isinstance(a.right, astConst)
        and isinstance(b.right, astConst)
    ...
    if (
        isinstance(a, astPower)
        and isinstance(b, astVar)
        and isinstance(a.left, astVar)
        and a.left.var == b.var
    ):
        return unifyMonomialFactors(a, b ** astConst(1))
    # x^n * x = x^( n + 1 )
    if (
        isinstance(b, astPower)
        and isinstance(a, astVar)
        and isinstance(b.left, astVar)
        and b.left.var == a.var
    ):
